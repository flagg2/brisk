...
export class Auth<Message, AuthResolverStyle extends "request" | "token"> {
   constructor(
      ...
      private rolesResolver: AuthResolverStyle extends "token" ? (decodedToken: JwtPayload) => Role[] : (req: Request) => Role[],
      private authResolverStyle: AuthResolverStyle = "token" as AuthResolverStyle
   ) {}
   public getMiddleware(allowedRoles: Role[]) {
      return (req: Request, res: ExtendedExpressResponse<Message>, next: NextFunction) => {
         ...
         if (this.authResolverStyle === "token") {
            ...
            try {
               ...
               decodedToken = jwt.verify(extractedToken, this.signingSecret) as JwtPayload;
            } catch (e) {
               return res.unauthorized();
            }
