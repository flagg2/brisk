...
export class BriskLogger extends Logger {
   public logRequest(config: { method: string; path: string; statusCode: number; durationMs: number; sizeKB: number }) {
      ...
      const message = `${method} ${path} ${durationColor(durationMs.toFixed(3) + "ms")} ${sizeColor(
         sizeKB + "KB"
